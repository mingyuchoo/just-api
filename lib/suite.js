...
import { assertFileValidity, loadModule, runModuleFunction, runInlineFunction } from './utils';
...
export default class Suite extends EventEmitter {
  ...
  async launch() {
    ...
    try {
      ...
      let error = new LoadingSpecDependencySuiteError(
        `${err.name} error occurred while loading dependencies \n ${err.message}`
      );
      this.status = 'fail';
    ...
    try {
      ...
      if (err.name === 'InvalidSuiteConfigurationError' || err.name === 'InvalidYAMLSuiteSchemaError') {
        ...
        let error = new SuiteConfigurationFailedError(
          `error occurred while configuring the suite '${this.file}' \n ${err.message}`
        );
        this.status = 'fail';
        this.emit('end', this, error);
      }
    ...
    if (suiteConfigData.custom_configuration) {
      ...
      try {
        if (suiteConfigData.custom_configuration.run_type === 'inline') {
          ...
          await runModuleFunction(
            customModule,
            suiteConfigData.custom_configuration.module.function_name,
            configContext
          );
        } else {
          throw new InvalidYAMLSuiteSchemaError(
            `suite custom_configuration.run_type should be either inline or module`
          );
        }
      } catch (err) {
        if (err.name === 'InvalidYAMLSuiteSchemaError') {
          ...
          throw new SuiteCustomConfigurationError(
            `${err.name || 'Error'} occurred while running the custom configuration function \n ${err.message || err}`
          );
        }
      }
  ...
  async fetchLoopItems(loopData) {
    ...
    try {
      ...
      if (err.name === 'LoopItemsBuilderError') {
        ...
        throw new LoopItemsBuilderError(
          `${err.name || 'Error'} occurred while building loop items \n ${err.message || err}`
        );
      }
    }
  ...
  async runBeforeAllHook() {
    if (this.hooks.before_all) {
      try {
        ...
        throw new BeforeAllHookError(
          `${err.name || 'Error'} occurred while running the before all hook \n    ${err.message || err}`
        );
      }
    }
  }
  ...
  async runAfterAllHook() {
    if (this.hooks.after_all) {
      try {
        ...
        throw new AfterAllHookError(
          `${err.name || 'Error'} occurred while running the after all hook \n     ${err.message || err}`
        );
      }
    }
  }
