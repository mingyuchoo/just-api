...
export default class Spec {
  ...
  async runCustomResponseValidator(response) {
    ...
    try {
      ...
      if (err.name === 'CustomResponseValidationError') {
        ...
        throw new CustomResponseValidationError(
          `${err.name || 'Error'} occurred in custom response validation \n ${err.message || err}`
        );
      }
    }
  }
  ...
  buildBaseURL(additionalOptions) {
    ...
    if (additionalOptions.outerDependency) {
      options = Object.assign(
        {},
        additionalOptions.outerDependencySuite.targetConfiguration,
        this.data.request,
        additionalOptions
      );
    } else {
  ...
  buildHeaders(additionalOptions) {
    ...
    let commonSuiteHeaders = this.data.ignore_suite_headers
      ? {}
      : additionalOptions.outerDependency
      ? additionalOptions.outerDependencySuite.commonHeaders
      : this.suite.commonHeaders;
  ...
  buildRequestBody(additionalOptions, headers, method) {
    ...
    if (requestContentType && requestContentType.length > 1) {
      ...
      if (!payload) {
        throw new RequestBodyNotFoundError(
          `request content-type is ${requestContentType}, but request body is not found`
        );
      }

      if (typeof payload !== 'object') {
        throw new RequestBodyNotFoundError(
          `request content-type is ${requestContentType}, invalid specification for request body. \n  Provide request body as one of ${expectedBodyFields} in payload `
        );
      }
      ...
      if (providedFields.length < 1) {
        throw new RequestBodyNotFoundError(
          `Request method is ${method}, but request specification does not have any expected body fields. \n  Provide request body as one of ${expectedBodyFields}`
        );
      }

      if (providedFields.length > 1) {
        throw new InvalidRequestSpecificationError(
          `request method is ${method}, and request specification contains more than one of expected body fields. Provide one of ${expectedBodyFields}`
        );
      }
        ...
        if (givenBody.type === 'binary') {
          ...
          try {
            ...
            if (fileExistsAndFile) {
              ...
              throw new RequestBodyBuilderError(
                `${file} is not a file, Provide a valid file path to be sent as body content`
              );
            }
          } catch (e) {
            if (e.code == 'ENOENT') {
              throw new RequestBodyBuilderError(
                `ENOENT file at '${file}' does not exist, Provide a valid file path to be sent as body content`
              );
            } else {
              throw e;
            }
          ...
          if (typeof givenBody.content === 'string' || typeof givenBody.content === 'number') {
            ...
            throw new InvalidRequestSpecificationError(
              `Payload type is given as text, but content provided is not a string or number`
            );
          }
          ...
          if (typeof givenBody.content === 'string' || typeof givenBody.content === 'number') {
            ...
            if (requestContentType === 'application/json') {
              ...
              throw new InvalidRequestSpecificationError(
                `Request body is an object but content-type header value is not application/json`
              );
            }
          } else {
            try {
              ...
              throw new RequestBodyBuilderError(
                `${error.name} Unable to dump the body content as json \n ${error.message}`
              );
            }
          }
        } else {
      ...
      if (providedBodyField === 'form') {
        if (requestContentType !== 'application/x-www-form-urlencoded') {
          throw new InvalidRequestHeaderError(
            `Request body is provided as form but request Content-Type is ${requestContentType}. It should be application/x-www-form-urlencoded`
          );
        }
      ...
      if (providedBodyField === 'form_data') {
        ...
        if (requestContentType !== 'multipart/form-data') {
          throw new InvalidRequestHeaderError(
            `Request body is provided as formData but request Content-Type is ${requestContentType}. It should be multipart/form-data`
          );
        }
          ...
          if (formParam.type === 'file') {
            ...
            try {
              ...
              if (fileExistsAndFile) {
                ...
                throw new RequestBodyBuilderError(
                  ` '${file}' is not a file, Provide a valid path for content of form body`
                );
              }
            } catch (e) {
              if (e.code == 'ENOENT') {
                throw new RequestBodyBuilderError(
                  `ENOENT file at '${file}' does not exist, Provide a valid path for content of form body`
                );
              } else {
                throw e;
              }
            ...
            if (typeof formParam.content === 'string' || typeof formParam.content === 'number') {
              ...
              throw new InvalidRequestSpecificationError(
                `Multipart form field type is given as text, but content provided is not a string or number`
              );
            }
      ...
      throw new InvalidRequestHeaderError(
        `Request method is ${method},request body is provided but Content-Type header is not provided`
      );
    } else {
      return bodyData;
    }
  ...
  _validateStatus(response, code) {
    if (code !== response._response.statusCode) {
      ...
      throw new ResponseStatusCodeDidNotMatchError(
        `Expected status code: ${code}, Actual status code: ${response._response.statusCode}`
      );
    }
  ...
  _validateHeaders(response, expectedHeaders) {
    ...
    for (let headerKey in headers) {
      ...
      if (typeof actualHeaderValue === 'undefined') {
        throw new ResponseHeaderValueDidNotMatchError(
          `Expected value for header: ${headerKey} is ${expectedHeaderValue}, Actual value: [couldn't find it in response headers]`
        );
      }
      ...
      if (expectedHeaderValue.constructor.name === 'RegExp') {
        ...
        if (!result) {
          throw new ResponseHeaderValueDidNotMatchError(
            `Header value did not match with expected Regex. Expected value for header: ${headerKey} is to match RegExp ${expectedHeaderValue}, Actual value: ${actualHeaderValue}`
          );
        }
      } else {
        if (expectedHeaderValue.toString() !== actualHeaderValue.toString()) {
          throw new ResponseHeaderValueDidNotMatchError(
            `Expected value for header: ${headerKey} is ${expectedHeaderValue}, Actual value: ${actualHeaderValue}`
          );
        }
      }
    }
  ...
  _validateCookies(response, expectedCookies) {
    ...
    for (let item in cookies) {
      ...
      if (typeof actualCookieDetails === 'undefined') {
        throw new ResponseCookieValueDidNotMatchError(
          `Expected value for Cookie: ${item} is ${expectedCookieDetails.value}, Actual value: [couldn't find it in response cookies]`
        );
      }
      ...
      if (expectedCookieDetails.value.constructor.name === 'RegExp') {
        ...
        if (!result) {
          throw new ResponseCookieValueDidNotMatchError(
            `Cookie value did not match with expected Regex. Expected value for cookie: ${item} is to match RegExp ${expectedCookieDetails.value}, Actual value: ${actualCookieDetails.value}`
          );
        }
      } else {
        if (expectedCookieDetails.value.toString() !== actualCookieDetails.value.toString()) {
          throw new ResponseCookieValueDidNotMatchError(
            `Expected value for cookie: ${item} is ${expectedCookieDetails.value}, Actual value: ${actualCookieDetails.value}`
          );
        }
      }
    }
  ...
  _validateJSONData(response, jsonData) {
    ...
    for (let eachJsonData of jsonData) {
      ...
      if (!values) {
        throw new ResponseJSONDataMismatchError(
          `JSON Path evaluation did not return any matching value, json path: ${eachJsonData.path}`
        );
      }
      ...
      if (eachJsonData.value === null) {
        if (values[0] !== eachJsonData.value) {
          throw new ResponseJSONDataMismatchError(
            `JSON path evaluated value did not match with expected value (null), json path: ${eachJsonData.path}, Actual value: ${values[0]}, Expected value: ${eachJsonData.value}`
          );
        }
      } else if (isNumber(eachJsonData.value) || eachJsonData.value.constructor.name === 'String') {
        if (values[0] !== eachJsonData.value) {
          throw new ResponseJSONDataMismatchError(
            `JSON path evaluated value did not match with expected value, json path: ${eachJsonData.path}, Actual value: ${values[0]}, Expected value: ${eachJsonData.value}`
          );
        }
        ...
        if (!result) {
          throw new ResponseJSONDataMismatchError(
            `JSON path evaluated value did not match the expected Regexp, json path: ${eachJsonData.path}, Regexp: ${eachJsonData.value}, Actual value: ${values[0]}`
          );
        }
      } else if (eachJsonData.value.constructor.name === 'Array') {
        if (!isEqual(values[0], eachJsonData.value)) {
          throw new ResponseJSONDataMismatchError(
            `JSON path evaluated value did not match with expected value, json path: ${
              eachJsonData.path
            }, Actual value: ${JSON.stringify(values[0])}, Expected value: ${JSON.stringify(eachJsonData.value)}`
          );
        }
        ...
        if (values[0] !== eachJsonData.value) {
          throw new ResponseJSONDataMismatchError(
            `JSON path evaluated value did not match with expected Boolean value, json path: ${eachJsonData.path}, Actual value: ${values[0]}, Expected value: ${eachJsonData.value}`
          );
        }
      } else {
        throw new Error(
          `Unknown json data value type, only Array, Object, RegExp, Number, Boolean & String are allowed`
        );
      }
    }
    ...
    try {
      if (jsonSchemaInfo.type === 'inline') {
        try {
          ...
          throw new ResponseJSONSchemaValidationError(
            `${e.name} occurred while parsing the inline input schema \n ${e.message} \n Input schema \n ${jsonSchemaInfo['$ref']}`
          );
        }
        ...
        try {
          ...
          throw new ResponseJSONSchemaValidationError(
            `${e.name} occurred while parsing the input schema in '${schemaFile}' \n ${e.message} \n Input schema: \n ${fileData}`
          );
        }
      } else {
      ...
      if (schemaValidation.errors.length >= 1) {
        ...
        for (let error of schemaValidation.errors) {
          errors.push(
            `Response json schema validation failed for property - ${error.property}, message: ${error.message}`
          );
        }
      ...
      if (err.name === 'ResponseJSONSchemaValidationError') {
        ...
        throw new ResponseJSONSchemaValidationError(
          `${err.name} occurred during response json schema validation \n ${err.message}`
        );
      }
    }
  }
