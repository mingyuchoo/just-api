...
export default class JustAPI {
  ...
  configureParallelism(input) {
    ...
    if (userInputForParallelism) {
      if (
        isNumber(userInputForParallelism) &&
        parseInt(userInputForParallelism) > 1 &&
        parseInt(userInputForParallelism) <= MAX_PARALLEL_LIMIT
      ) {
        this.parallel = true;
        this.max_parallel = parseInt(userInputForParallelism);
      } else {
        console.error(
          `Given argument for parallel option ${input} is invalid, Please provide a number (> 1 and <= ${MAX_PARALLEL_LIMIT})`
        );
        process.exit(1);
      }
    } else {
